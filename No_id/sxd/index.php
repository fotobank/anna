...
class Sypex_Dumper {
	...
	function init($args = false){
		...
		if ($args) { // консольный режим
		    ...
		    if($auth && !empty($this->CFG['sjob'])){
				...
				echo file_get_contents($this->JOB['fileLog']);
				if (file_exists($this->JOB['fileLog'])) {
					unlink($this->JOB['fileLog']);
				}
	...
	function addDb($req) {
		...
		if ($r) echo "sxd.addOpt(".sxd_php2json(array('db' => array($req['name'] => "{$req['name']} (0)"))).");"; else
			sxd_my_error();
	}
	...
	function addBackupJob($job) {
		...
		$this->JOB['fileLog']  = $this->CFG['backup_path'].$this->JOB['job'].'.log';
		...
		$this->fh_log = fopen($this->JOB['fileLog'], 'wb');
...
//						$this->setNames($this->JOB['correct'] == 1 && !empty($this->JOB['charset']) ? $this->JOB['charset'] : empty($m[3]) ? '' : $m[3]);
	...
	function resumeJob($job) {
		...
		$this->fh_log = fopen($this->JOB['fileLog'], 'ab');
		$t            = fgets($this->fh_rtl);
		if (!empty($t)) {
			$this->rtl = explode("\t", $t);
		} else {
	...
	function main() {
		...
		$this->VAR['combos'] = $this->addCombo('backup_db', $this->db, 11, 'db', array()/*$this->getDBList()*/).$this->addCombo('backup_charset', 0, 9, 'charset', $this->getCharsetList()).$this->addCombo('backup_zip', 7, 10, 'zip', $zip).$this->addCombo('restore_db', $this->db, 11, 'db').$this->addCombo('restore_charset', 0, 9, 'charset').$this->addCombo('restore_file', 0, 12, 'files', $this->getFileList()).$this->addCombo('restore_type', 0, 13, 'types', array(
				...
				'QUICK',
				'EXTENDED'
			)).$this->addCombo('db_charset', 0, 9, 'collation', $this->getCollationList()).$this->addCombo('db_charset_col', 0, 15, 'collation:db_charset');
		if (!V_MYSQL) {
