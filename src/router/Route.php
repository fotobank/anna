...
class Route {
     ...
     * @throws \Exception
     */
    public function __construct($pattern, array $dispatch = []) {
        try
        {
     ...
     *
     * @return \router\Route A copy of the route object with the matches populated, or false on non-match.
     * @throws \Exception
     */
    ...
    protected function initialize($pattern, array $dispatch) {
        try
        {
            ...
            $parts = array_map([$this, 'parsePart'], $parts);
            if($parts)
            {
    ...
    protected function parsePart($part) {
        ...
        return [$match['name'] => $match['pattern']];
    }
