...
Этот класс может объединить и уменьшать и JavaScript и CSS файлы. Она может сжимать наличие и CSS файлы, удалив ненужные комментарии, пробелы, вкладки и разрывы строк. Ранее уменьшенная изображения пропускаются. обработки Javascript можете использовать JSMin или API Google Закрытие. Класс может также объединить несколько JavaScript и CSS файлы минимизировать количество запросов к серверу. Внешние изображения, используемые CSS-файлов могут быть преобразованы для вставки в стили CSS, используя URL-адресов данных. Кэши класса обработанные файлы, чтобы избежать затраты на обработку, пока любой из исходных файлов будет изменен.
