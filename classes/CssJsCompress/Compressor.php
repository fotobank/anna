...
class Compressor {
	 ...
	 *                       - closure - Use google closure (utilizes cURL)
	 */


	protected $config_keys = array(
		'echo'                   => true, //Return or echo the values
		'encode'                 => true, //base64 images from CSS and include as part of the file?
		'timer'                  => true, //Ouput script execution time
		'gzip'                   => false, //Output as php with gzip?
		'packer'                 => 'closure', //Use google closure (utilizes cURL) // closure / jsminplus / jsmin / packer / minifier ''
		'remove_comments'        => true, // remove comments,
		'hashed_filenames'       => false, //Generate hashbased filenames to break caches
		'optimize_javascript_gz' => true, // Use gzip compression on Javascript Cache files . Compress Javascript files - size reduction.
		'optimize_css_gz'        => true // Use gzip compression on CSS Cache files. File size reduction.
	);
	 ...
	 * @param $src_file
	 */
	protected function css_packers( $src_file ) {
	...
	protected function css_clean_packers($src_file) {
	 ...
	 * @return string
	 */
	protected function gmstamp( $time = '' ) {
	 ...
	 */
	protected function minified_filedata( $source_file, $reference_file, $regen = false ) {
			...
			foreach ( $this->matches as $this->graphic ) {
				...
				if ( $this->remote_file( $this->graphic[2] ) ) {
					...
					if ( curl_exec( $ch ) !== FALSE ) {
						...
						if ( $this->filesize <= self::IMAGE_MAX_SIZE * 1024 ) {
...
//				$this->css_packers( $src_file );
				$this-> css_clean_packers( $src_file );
	...
	public function js_packers(){
		// Set timeout
		set_time_limit(2500);

		switch($this->settings['packer']){
			case 'packer':

				$this->content = $this->Packer();

				break;
			case 'jsmin':

				$this->content = $this->JSmin();

				break;

			case 'jsminplus':

				$this->content = $this->JSminplus();

				break;
			case 'minifier':

				$this->content = $this->Minifier();

				break;

			case 'closure':

				$this->content = $this->Closure();
	...
	public function Packer(){
		...
		$packer = new JavaScriptPacker($this->source, 'Normal', true, false);
		return $packer->pack();
	...
	public function Minifier(){
		...
		return Minifier::minify( $this->source, array( 'flaggedComments' => $this->settings['remove_comments'] ) );
	 ...
	 * @return bool|string
	 */
	public function JSminplus(){

		return JSMinPlus::minify( $this->source, array( 'flaggedComments' => $this->settings['remove_comments'] ) );
	 ...
	 * If bool is passed for 'packer' => 'closure' during class initiation, cURL request processes
	 *
	 *
	 */
	 ...
	 */
	protected function make_min( $src_file, $new_file, $do_minify = true ) {
		global $messages;
	...
	public function merge( $output_filename, $directory, $list_or_type = 'js', $exclude = array(), $order = array() ) {
		...
		if ( ! empty( $order ) ) {
			...
			foreach ( $order as $this->file ) {
				...
				if ( file_exists( $minified_name ) && ( $this->file != $output_filename ) && ( ! $this->remote_file( $this->file ) )
					&& ( $this->gmstamp( filemtime( $this->file ) ) > $minfile->filemtime )
				) {
	...
	public function logs() {
		...
		if ( ! empty( $this->messages ) ) {
			...
			foreach ( $this->messages as $this->data ) {
				foreach ( $this->data as $this->type => $this->console ) {
					echo 'console.log("' . $this->type . ' : ' . $this->console . '");' . PHP_EOL;
				}
			}
			echo '</script>';
	...
	public function include_packer() {
		switch ( $this->settings['packer'] ) {
			case 'packer':
				...
				require( __DIR__ . '/vendor/JavaScriptPacker/JavaScriptPacker.php' );

				break;
			case 'jsmin':

				require( __DIR__ . '/vendor/JSMin/JSMin.php' );

				break;

			case 'jsminplus':

				require( __DIR__ . '/vendor/JSMinPlus/jsminplus.php' );
