...
class Comments_Template {
	...
	protected $WIN32 = false; //	Set to true if this is a WIN32 server

	protected $ERROR = ""; //	Holds the last error message

	protected $LAST = ""; //	Holds the HANDLE to the last
	//	template parsed by parse()

	protected $STRICT = true; //	Strict template checking.
	...
	public function show_unknowns( $Line ) {
		...
		if ( preg_match( "/({[A-Z0-9_]+})/", $Line, $unknown ) ) {
			...
			if ( ! ( empty( $UnkVar ) ) ) {
				...
				trigger_error( "[FastTemplate] Warning: no value found for variable: $UnkVar ", E_USER_NOTICE );
			}
		}
	}
	...
	public function parse_template( $template, $tpl_array ) {
		...
		if ( ! $this->STRICT ) {
			...
			if ( preg_match( "/({[A-Z0-9_]+})/", $template ) ) {
				{
					$unknown = explode( "\n", $template );
					while ( list ( , $Line ) = each( $unknown ) ) {
						if ( ! ( empty( $Line ) ) ) {
							$this->show_unknowns( $Line );
						}
					}
				}
			}
...
//	The meat of the whole class. The magic happens here.
	...
	public function parse_dynamic( $Macro, $MacroName ) {
		...
		if ( $this->$ParentTag ) {
			...
			$start     = false;
			$end       = false;
			while ( list ( , $lineData ) = each( $DataArray ) ) {
			...
			trigger_error( "ParentTag: [$ParentTag] not loaded!", E_USER_NOTICE );
		}
		return false;
	}
...
//	Strips a DYNAMIC BLOCK from a template.
	...
	public function clear( $ReturnVar = "" ) {
		...
		while ( list ( $key, ) = each( $this->HANDLE ) ) {
			$KEY = $key;
			unset( $this->$KEY );
		}
		return;
	...
	public function clear_tpl( $fileHandle = "" ) {
		...
		if ( empty( $fileHandle ) ) {
			...
			while ( list ( $key, ) = each( $this->LOADED ) ) {
				unset( $this->$key );
			}
			unset( $this->LOADED );
			...
			if ( ( gettype( $fileHandle ) ) != "array" ) {
				...
				while ( list ( $Key, ) = each( $fileHandle ) ) {
...
//	Clears all variables set by assign()
	...
	public function clear_assign() {
		if ( ! ( empty( $this->PARSEVARS ) ) ) {
			while ( list( $Ref, ) = each( $this->PARSEVARS ) ) {
