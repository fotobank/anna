	...
	function show_unknowns ($Line)
	{
		...
		if (preg_match("/({[A-Z0-9_]+})/",$Line,$unknown))
		{
			...
			if(!(empty($UnkVar)))
			{
				...
				trigger_error("[FastTemplate] Warning: no value found for variable: $UnkVar ", E_USER_WARNING);
			}
		}
	}	// end show_unknowns()
...
//	************************************************************
	...
	function parse_dynamic ($Macro,$MacroName)
	{
		...
		else
		{
			...
			trigger_error("ParentTag: [$ParentTag] not loaded!", E_USER_WARNING);
		}
		return false;
	}
