 ...
 * Mysqli_Db Class
 *
 * изменена функция insert - добавил игноре
 *
...
class Mysqli_Db {
	...
	protected $db;
	protected $port;
	protected $cp = 'cp1251'; // utf-8

	protected $_transaction_in_progress; // добавил не была объявленна
	protected static $_file_pass = "/../../inc/passwords.php"; // данные для подключения
	...
	public function connect() {
		...
		$this->_mysqli->set_charset( $this->cp );
	}
    ...
    if(strlen($file) > 0) {
		self::$_file_pass = $file;
			...
			if ( is_readable( __DIR__ .  self::$_file_pass ) ) {
				$handle = file( __DIR__ . self::$_file_pass , FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES );
				array_shift($handle);
				if ( $handle ) {
					foreach ( $handle as $data ) {
						$rec = explode( "=>", $data );
						if ( count( $rec ) === 2 ) {
							self::$config[trim( $rec[0] )] = trim( $rec[1] );
						}
					}
				} else {
					trigger_error( "Не удалось открыть файл!" , E_USER_ERROR );
					return false;
				}
			} else {
	...
	public static function getInstance( $config = NULL ) {
		if (self::$_instance == null){
			self::$_instance = new Mysqli_Db( $config );
	...
	/**
	 * проверка и реинициализация соединения с базой
	 */
	 ...
	 *
	 * @uses $Mysqli_Db->where('id', 7)->where('title', 'MyTitle');
	 *
	 ...
	 *
	 * @uses $Mysqli_Db->orWhere('id', 7)->orWhere('title', 'MyTitle');
	 *
	 * @param string $whereProp  The name of the database field.
	 * @param mixed  $whereValue The value of the database field.
	 *
	 ...
	 *
	 * @uses $Mysqli_Db->join('table1', 'field1 <> field2', 'LEFT')
	 *
	 ...
	 *
	 * @uses $Mysqli_Db->orderBy('id', 'desc')->orderBy('name', 'desc');
	 *
	...
	protected function _buildPair( $operator, $value ) {
		...
		$subQuery   = $value->$this->getSubQuery();
	...
	/**
	 * Method creates new Mysqli_Db object for a subquery generation
	 */
	public static function subQuery() {
		return new Mysqli_Db();
	}

	/**
	 * Method returns a copy of a Mysqli_Db subquery object
	 *
	 * @param object /new Mysqli_Db object
	 */
	/**
	 * @return Mysqli_Db
	 */
