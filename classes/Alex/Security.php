...
class Security
{
	...
	function karo($t = false)
		{
			...
			if ($this->is_server($this->getRealIP())) {
				$this->send403();
			...
			if ((false === ( bool ) $t) || (false !== ( bool ) $this->_bypassbanip)) {
				...
				if ((false !== $t) && (false !== $this->hCoreFileChk($this->_htaccessfile, true, true))) {
				...
				while ($j <= 6) {
                  ...
                  \bval\b|0x|--";
                   ...
                   \bsql\b|tmp|tmp\/sess|\busr\b|\bvar\b|\/(?:uploa|passw)d';
					...
					if ((false !== ( bool ) preg_match("/onmouse(?:down|over)/i", $string)) && (2 <
																								( int ) preg_match_all("/c(?:path|tthis|t\(this)|http:|(?:forgotte|admi)n|sqlpatch|,,|ftp:|(?:aler|promp)t/i",
																													   $thenode,
																													   $matches))
					) {
						return true;
                  ...
                  request|xor)|(?:column|db|load|not|octet|sql|table|xp)_';
					...
					if ((false !== ( bool ) preg_match("/\border by\b|\bgroup by\b/i", $string)) &&
						(false !== ( bool ) preg_match("/\bcolumn\b|\bdesc\b|\berror\b|\bfrom\b|hav|\blimit\b|offset|\btable\b|\/|--/i",
																														 $string) ||
						...
						return true;
							   ...
							   (false !== strpos($string, 'select') && (false !==
	...
	function _COOKIE_SHIELD()
		{
			...
			while ($i < count($ckeys)) {
				...
				if ((is_string($ckey))) {
					if (false !== ( bool ) $this->blacklistMatch($ckey, 4) ||
						false !== ( bool ) $this->blacklistMatch($this->hexoctaldecode($cval), 4)
					) {
						$this->karo(true);
	...
	function getPHP_SELF()
		{
			...
			if (is_array($matches) && array_key_exists(0, $matches) && ('.php' == substr($matches[0], - 4, 4)) &&
				(false !== $this->checkfilename($matches[0])) && (is_readable($matches[0]))
			) {
				$filename = $matches[0];
			} else {
				$filename = null;
	...
	private static function setReq_uri()
		{
			$_request_uri = '';
			if (empty($_SERVER['REQUEST_URI']) || (php_sapi_name() != 'cgi-fcgi' && false !==
																					( bool ) preg_match("/^Microsoft-IIS\//",
